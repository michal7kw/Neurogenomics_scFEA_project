...
geneExpr = pd.read_pickle('./input/geneExpr2.pkl')
geneExprScale = pd.read_pickle('./input/geneExprScale2.pkl')
...
geneExprDf = pd.read_pickle('./input/geneExprDf.pkl')
...
with open("./output/lossValue_" + timestr + ".txt", "a") as file_loss:
    ...
    for epoch in tqdm(range(EPOCH)):
        ...
        for i, (X, X_scale, m_scale) in enumerate(train_loader):
            ...
            X_scale_batch = X_scale.to(device).float()
...
with open('./' + res_dir + '/time_' + timestr + ".txt", "w") as f:
